  ...
  describe("General Tests.", () => {
    createServer([
      ...
      },
      {
        ...
        method: "get",
        res: () => {
          return mockProfileReposRequest;
        },
      },
    ]);
    ...
    test("A profile should be rendered when you enter the profile name in the input and click submit.", async () => {
      ...
      await user.keyboard(mockProfileRequest.login);

      await user.click(btnSubmit);

      const img = screen.getByRole("img");
      const name = screen.getByRole("heading", {
        name: mockProfileRequest.name,
      });
      const description = screen.getByText(mockProfileRequest.bio);
      ...
      expect(img).toHaveAttribute("src", mockProfileRequest.avatar_url);
      expect(img).toHaveAttribute("alt", mockProfileRequest.name);
      ...
      expect(listRepos.children).toHaveLength(mockProfileReposRequest.length);
    });
  });
});
